...
E'_id  = Softmax(Q_id K_sem^T / sqrt(d)) V_sem   [Eq. 23]
E'_sem = Softmax(Q_sem K_id^T / sqrt(d)) V_id    [Eq. 24]
    ↓ Concatenation + W_f
E_XStream ∈ R^{B×L×d}
    ↓ Cross-attention with U-Net features F_{U-Net}
F_fus_{U-Net} = F_{U-Net} + Reshape(F_att W_o)   [Eq. 30]
```
...
Metrics computed: IDsim (ArcFace), Semdist (DINO), EXPAcc (DAN FER), Poseerr (yaw/pitch/roll), FID.
...
@article{afgan2026videxpo,
  ...
  year      = {2026},
  note      = {Preprint submitted June 7, 2026},
}
```
